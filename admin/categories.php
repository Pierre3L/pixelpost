...
echo "<div id='caption'>$admin_lang_categories</div>";
    ...
    $query = sql_query($db, "delete from ".$pixelpost_db_prefix."categories where id='".$_POST['id']."'");
    ...
    if($_GET['action'] == "add") {
        ...
        if (isset($_POST['alt_category'])) {
		    ...
		    $query = sql_query($db, "insert into ".$pixelpost_db_prefix."categories(id,name,alt_name) VALUES(NULL,'$category','$alt_category')");
  		   echo "<div class='content confirm'>$admin_lang_done $admin_lang_cats_added<p /></div>
	    ";
        }
    if($_GET['action'] == "edit") {
	$query = sql_array($db, "select * from ".$pixelpost_db_prefix."categories where id='".$_POST['id']."'");
    ...
    if($_GET['action'] == "update") {
        ...
        $getid = $_GET['id'];
        $upquery = sql_query($db, "update ".$pixelpost_db_prefix."categories set name='$category', alt_name='$alt_category' where id='$getid'");
