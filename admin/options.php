...
if($_GET['view'] == "options") {
	...
	if ($_GET['optaction']=='updateall'){
		if($_GET['optionsview']=='' OR $_GET['optionsview']=='general'){
			...
			if(!get_magic_quotes_gpc()){
				...
				$update = sql_query($db, "update ".$pixelpost_db_prefix."config set sitetitle='".addslashes($_POST['new_site_title'])."' where admin='".$cfgrow['admin']."'");
			}else{
				$update = sql_query($db, "update ".$pixelpost_db_prefix."config set sitetitle='".($_POST['new_site_title'])."' where admin='".$cfgrow['admin']."'");
			}
			//break;
			...
			if(!get_magic_quotes_gpc()){
				// we need to escape the string before saving it to the db
				$update = sql_query($db, "update ".$pixelpost_db_prefix."config set siteurl='".addslashes($_POST['new_site_url'])."' where admin='".$cfgrow['admin']."'");
			}else{
				$update = sql_query($db, "update ".$pixelpost_db_prefix."config set siteurl='".($_POST['new_site_url'])."' where admin='".$cfgrow['admin']."'");
			}
			//break;
			...
			if ($_POST['newadminpass']!= ''){
				if ($_POST['newadminpass_re'] == $_POST['newadminpass']){
					...
					$update = sql_query($db, "update ".$pixelpost_db_prefix."config set admin='".$_POST['new_admin_user']."', password='$new_pass' where admin='".$cfgrow['admin']."'");
			...
			if ($_POST['new_lang']!=$_POST['alt_lang']){
				$update = sql_query($db, "update ".$pixelpost_db_prefix."config set langfile='".$_POST['new_lang']."', altlangfile='".$_POST['alt_lang']."' where admin='".$cfgrow['admin']."'");
				$lang_error=0;
			}else{
				$lang_error=1;
			}
			...
			//case "updateemail":
			if ($_POST['new_email']!='')
				$update = sql_query($db, "update ".$pixelpost_db_prefix."config set email='".$_POST['new_email']."' where admin='".$cfgrow['admin']."'");
			//break;

			//case "updateimagepath": // imagepath
			$update = sql_query($db, "update ".$pixelpost_db_prefix."config set imagepath='".$_POST['new_image_path']."' where admin='".$cfgrow['admin']."'");
			//break;

			//case "updatetimezone":
			$upquery = sql_query($db, "update ".$pixelpost_db_prefix."config set timezone='".$_POST['timezone']."'");
			//break;

			//case "updateallowcomments":
			$upquery = sql_query($db, "update ".$pixelpost_db_prefix."config set global_comments='".$_POST['global_comments']."'");
			//break;

			//case "updatecommentemail":
			$update = sql_query($db, "update ".$pixelpost_db_prefix."config set commentemail='".$_POST['new_commentemail']."' where admin='".$cfgrow['admin']."'");
			//break;

			//case "updatehtmlemailnote":
			$upquery = sql_query($db, "update ".$pixelpost_db_prefix."config set htmlemailnote='".$_POST['new_htmlemailnote']."'");
			//break;

			// time stamps
			$upquery = sql_query($db, "update ".$pixelpost_db_prefix."config set timestamp='".$_POST['timestamp']."'");

			// book visitors
			$upquery = sql_query($db, "update ".$pixelpost_db_prefix."config set visitorbooking='".$_POST['visitorbooking']."'");

			// markdown
			$upquery = sql_query($db, "update ".$pixelpost_db_prefix."config set markdown='".$_POST['markdown']."'");

			// exif
			$upquery = sql_query($db, "update ".$pixelpost_db_prefix."config set exif='".$_POST['exif']."'");
	...
	switch ($_GET['optaction']){
		...
		case "updatetemplate":
			$update = sql_query($db, "update ".$pixelpost_db_prefix."config set template='".$_POST['new_template']."' where admin='".$cfgrow['admin']."'");
		break;

		case "updatedateformat":
			$update = sql_query($db, "update ".$pixelpost_db_prefix."config set dateformat='".$_POST['new_dateformat']."'");
		break;
		...
		case "updatecatformat":
			...
			$upquery = sql_query($db, "update ".$pixelpost_db_prefix."config set catgluestart='" .$startcatformat ."', catglueend='" .$endcatformat ."'");
		break;

		case "updatecalendar":
			$update = sql_query($db, "update ".$pixelpost_db_prefix."config set calendar='".$_POST['cal']."'");
		break;
		//=========== END OF PAGE TWO: TEMPLATE ===========

		//=========== START PAGE THREE: THUMBNAIL ===========
		case "updatethumbrow":
			$update = sql_query($db, "update ".$pixelpost_db_prefix."config set thumbnumber='".$_POST['thumbnumber']."' where admin='".$cfgrow['admin']."'");
		break;

		case "updatecrop":
			$update = sql_query($db, "update ".$pixelpost_db_prefix."config set crop='".$_POST['new_crop']."' where admin='".$cfgrow['admin']."'");
		break;
		...
		case "updatecompression":
			$update = sql_query($db, "update ".$pixelpost_db_prefix."config set compression='".$_POST['new_compression']."' where admin='".$cfgrow['admin']."'");
		break;
	...
	if ($_GET['optionsview']=='template'){
		...
		";
		if($cfgrow['calendar'] == "") { 
			$nice_calendar = "No Calendar"; 
		} else { 
			$nice_calendar = $cfgrow['calendar']; 
		}
		echo "
    ...
    if ($_GET['optionsview']=='antispam'){
		if ($_GET['optaction']=='updateantispam'){
			// token
			$upquery = sql_query($db, "update ".$pixelpost_db_prefix."config set token='".$_POST['token']."', token_time='".(int) $_POST['token_time']."'");
			// DSBL settings
			$upquery = sql_query($db, "update ".$pixelpost_db_prefix."config set comment_dsbl='".$_POST['comment_dsbl']."'");
			// SPAM Flood settings
			$upquery = sql_query($db, "update ".$pixelpost_db_prefix."config set comment_timebetween='".(int) $_POST['comment_timebetween']."'");
...
function show_anti_spam($db){
	// update it if neccessary
	$additional_msg = update_banlist($db);
	$additional_msg .= moderate_past_with_list($db);
	$additional_msg .= delete_past_with_list($db);
	$additional_msg .= delete_from_badreferer_list($db);
	$html = options_anti_spam_html($db, $additional_msg);
	echo $html;
}

// create options HTML code
function options_anti_spam_html($db, $additional_msg){
	...
	$mod_list   = get_moderation_banlist($db);
	$black_list = get_blacklist($db);
	$ref_list = get_ref_ban_list($db);
	$query = "SELECT acceptable_num_links FROM {$pixelpost_db_prefix}banlist LIMIT 1";
	$result = mysqli_query($db, $query) or die( mysqli_error($db));
	if( $row = mysqli_fetch_row($result))
		$acceptable_num_links = $row[0];
	// htaccess stuff
	$htaccess = create_htaccess_banlist($db);
	$HTML = "
		<div class='jcaption'>
		$admin_lang_spam_ban
		</div>
